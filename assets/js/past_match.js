...
// for loop for completed match
  ...
  for (let i = 0; i < completed_match_list.length; i++) {
    ...
    const find = match_list.find((e) => {
      ...
      const ans = e.matchUniqueId == check
...
function completedMatchTemp(match_data, my_team, opp_team, won, loss, draw) {
  const temp = 	`<div class="past_result_container" data-id="${match_data.matchUniqueId}">
...
for (let i = 0; i < completed_match.length; i++) {
  ...
  const my_team_response = request_list.find((e) => e.matchUniqueId == match_data.matchUniqueId && e.team_players.find((e) => e == phonenumber) == phonenumber);// some error is there
  console.log(my_team_response);
  const my_team_score = score_resopnse.find((e) => e.matchUniqueId == match_data.matchUniqueId && e.teamUniqueId == my_team_response.team_id);

  const my_team_obj = team_list.find((e) => e.uniqueId == my_team_response.team_id);
  // console.log(my_team_score)

  const opp_team_response = request_list.find((e) => e.matchUniqueId == match_data.matchUniqueId && e.team_id != my_team_response.team_id);

  const opp_team_score = score_resopnse.find((e) => e.matchUniqueId == match_data.matchUniqueId && e.teamUniqueId == opp_team_response.team_id);

  const opp_team_obj = team_list.find((e) => e.uniqueId == opp_team_response.team_id);
  // console.log(opp_team_score)
  let won = 0;
  let loss = 0;
  let draw = 0;

  let my_draw = 'nan'; let my_loss; let my_won; let opp_draw = 'nan'; let opp_loss; let
    opp_won;
  ...
  const func = completedMatchTemp(match_data, my_team_obj, opp_team_obj, won, loss, draw);
...
pastdetails.forEach((bookCover) => {
  bookCover.addEventListener('click', (event) => {
    ...
    const my_team_response = request_list.find((e) => e.matchUniqueId == person_data && e.team_players.find((e) => e == phonenumber) == phonenumber);

    const my_team_score = score_resopnse.find((e) => e.matchUniqueId == person_data && e.teamUniqueId == my_team_response.team_id);

    const my_team_obj = team_list.find((e) => e.uniqueId == my_team_response.team_id);

    const opp_team_response = request_list.find((e) => e.matchUniqueId == person_data && e.team_id != my_team_response.team_id);

    const opp_team_score = score_resopnse.find((e) => e.matchUniqueId == person_data && e.teamUniqueId == opp_team_response.team_id);
    ...
    if (my_team_score) {
      for (let i = 0; i < (my_team_score.mvps).length; i++) {
        const find_player = players_list.find((e) => e.phoneNumber == my_team_score.mvps[i]);
    ...
    if (opp_team_score) {
      for (let i = 0; i < (opp_team_score.mvps).length; i++) {
        const find_player = players_list.find((e) => e.phoneNumber == opp_team_score.mvps[i]);
    ...
    for (let i = 0; i < (my_team_response.team_players).length; i++) {
      const find_player = players_list.find((e) => e.phoneNumber == my_team_response.team_players[i]);
    ...
    for (let i = 0; i < (opp_team_response.team_players).length; i++) {
      const find_player = players_list.find((e) => e.phoneNumber == opp_team_response.team_players[i]);
      const func = pastplayersTemp(find_player);
