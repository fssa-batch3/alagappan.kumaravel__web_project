...
// let completed_match_list; // bug shown but i  used in pastmatch.js
// if (request_list) {
...
if (request_list !== []) {
    ...
    console.log(matchInvitaton)
     ...
     live_match = match_request_list.filter(
        e =>{
...
const completete_match_req_id = await getMyMatchesReq(my_id);
...
if (matchInvitaton !== 0) {
    ...
    const opponent_all_req = await getMatchResList("match_response_list", matchInvitaton[i]["id"])
    ...
    const captain_id = await findCaptainId(opponent_team_id["team_id"])

    const captain_object = await getDataById("users", captain_id[0]["playerId"])

    const captain = captain_object["userName"]; 
